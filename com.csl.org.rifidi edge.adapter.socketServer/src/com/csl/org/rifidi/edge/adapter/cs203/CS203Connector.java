...
import com.csl.org.rifidi.multiplereader.S_EPC;
import com.csl.org.rifidi.multiplereader.S_PC;
import com.csl.org.rifidi.multiplereader.TagCallbackInfo;

public class CS203Connector {
	...
	private CS203ReaderSession session ;
	...
	public CS203Connector(String deviseName, String ip) {
		this.deviceName = deviseName;
		this.TagBuffer = new LinkedList<TagInfo>();
		this.setIpAddress(ip);
	}
	
	public CS203Connector(String deviceName) {
		this.deviceName = deviceName;
	}

	public boolean Connect() {
		return this.Connect(this.getIpAddress());
	}
	...
	public boolean Connect(String ip) {
		...
		try {
			...
			// open cport (1516)
			TCPCtrlSocket = new Socket(getIpAddress(), 1516);
			...
			System.out
					.println("Enable TCP Notifications with command 0x8000011701");
			while (true) {
			...
			// open iport (1515)
			TCPDataSocket = new Socket(getIpAddress(), 1515);
	...
	public void StartInventory() {
		...
		this.StartInventory(this.ipAddress);
	}
			...
			while (!stopInventory) {
				try {
					...
					PrintWriter out = new PrintWriter(new BufferedWriter(
							new FileWriter(getIpAddress() + ".txt", true)));
				...
				if (port != 0) {
					...
					System.out
							.println("Send ANT_PORT_SEL command 0x7001010700000000");
					Thread.sleep(1);

					TCPDataOut.write(hexStringToByteArray("7001020700000000"));
					System.out
							.println("Send ANT_PORT_CFG command 0x7001020700000000");
					Thread.sleep(1);

					cmdString = String
							.format("70010107%02X000000", port & 0xFF);
					TCPDataOut.write(hexStringToByteArray(cmdString));
					System.out.println("Send ANT_PORT_SEL command 0x"
							+ cmdString);
					Thread.sleep(1);

					TCPDataOut.write(hexStringToByteArray("7001020701000000"));
					System.out
							.println("Send ANT_PORT_CFG command 0x7001020701000000");
					Thread.sleep(1);
				} else {
					// Select Antenna port ANT_PORT_SEL
					TCPDataOut.write(hexStringToByteArray("7001010700000000"));
				...
				cmdString = String.format("70010607%02X%02X0000", power & 0xFF,
						((power >> 8) & 0xFF));
				// TCPDataOut.write(hexStringToByteArray("700106072C010000"));
				TCPDataOut.write(hexStringToByteArray(cmdString));
				System.out.println(String.format(
						"Set RF Power to %4.1f dBm with command %s",
						(float) power / 10, cmdString));

				Thread.sleep(1);
				// Set Link Profile 2
				TCPDataOut.write(hexStringToByteArray("7001600b02000000"));
				System.out
						.println("Set Link Profile 2 command 7001600b02000000");
				...
				System.out
						.println("QUERY_CFG (continuous mode) command 70010007ffff0000");
				...
				if (Country == 1 || Country == 3 || Country == 8) {
					...
					for (int i = 0; i <= 49; i++) {
						TCPDataOut.write(hexStringToByteArray(String.format(
								"7001010C%02X000000", i)));
						Thread.sleep(1);
						TCPDataOut
								.write(hexStringToByteArray("7001020C00000000"));
						Thread.sleep(1);
					}
					// enable channel 1
					...
					for (int i = 0; i < FCC_CHN_CNT; i++) {
						...
						Thread.sleep(1);
						TCPDataOut
								.write(hexStringToByteArray("7001020C01000000"));
						Thread.sleep(1);
						TCPDataOut.write(hexStringToByteArray(String.format(
								"7001030C%08X",
								fccFreqTable[fccFreqSortedIdx[i]])));
						Thread.sleep(1);
					}
				} else if (Country == 4) {
					// Enable channels
					for (int i = 0; i < TW_CHN_CNT; i++) {
						...
						TCPDataOut
								.write(hexStringToByteArray("7001020C01000000"));
						Thread.sleep(1);
						TCPDataOut.write(hexStringToByteArray(String
								.format("7001030C%08X",
										twFreqTable[twFreqSortedIdx[i]])));
						Thread.sleep(1);
					}
					// Disable channels
				...
				System.out
						.println("Set DynamicQ algorithm (INV_SEL) command 7001020903000000");
				Thread.sleep(1);
				...
				System.out
						.println("Start inventory - send (HST_CMD) 700100f00f000000");
				while (true) {
					if (TCPDataIn.available() >= 8) {
						...
						if (byteArrayToHexString(inData, len).startsWith(
							...
							continue;
						} else if (byteArrayToHexString(inData, len)
								.startsWith("02000780")
								|| byteArrayToHexString(inData, len)
										.startsWith("01000780")) {
							date = new Date();
							...
							continue;
								...
								.startsWith("4003BFFCBFFCBFFC")) {
							// Check Abort command response
							System.out
									.println("All tag data has been returned");
							break;
						}
						...
						while (TCPDataIn.available() < datalen) {
							if (System.currentTimeMillis() - timer >= 3000) {
								System.out
										.println("Incomplete packet returned.  Packet header: "
												+ byteArrayToHexString(inData,
														len));
								len = TCPDataIn.read(inData, 0,
										TCPDataIn.available());
							...
							try {
								...
								PrintWriter out = new PrintWriter(
										new BufferedWriter(new FileWriter(
												getIpAddress() + ".txt", true)));
								out.println(dateFormat.format(date)
										+ " Incomplete data packet due to reader connection lost");
								out.close();
							} catch (IOException e) {
								System.out
						...
						if (pkt_type == 0x8005 || pkt_type == 0x0005) {
							...
							AsyncCallbackRaiseEvent(new TagCallbackInfo(rssi,
									new S_PC(byteArrayToHexString(PC, 2)),
									new S_EPC(byteArrayToHexString(EPC,
											datalen - 16)), getIpAddress(),
									deviceName));
	...
	private void AsyncCallbackRaiseEvent(TagCallbackInfo data) {
		for (int i = 0, size = AsyncCallbackEventSubscribers.size(); i < size; i++) {
			((AsyncCallbackEventListener) AsyncCallbackEventSubscribers.get(i))
					.AsyncCallbackEvent(new AsyncCallbackEventArgs(this, data));
	...
	private int[] fccFreqTable = new int[] { 0x4F0E1800, /* 915.75 MHz */
	...
	private int[] fccFreqSortedIdx = new int[] { 26, 25, 1, 48, 47, 3, 49, 35,
			33, 13, 32, 30, 5, 4, 45, 38, 24, 8, 22, 39, 17, 18, 2, 12, 6, 19,
			7, 29, 23, 9, 31, 27, 15, 16, 10, 44, 14, 34, 28, 21, 42, 11, 46,
			20, 43, 37, 36, 40, 0, 41, };

	private int[] twFreqTable = new int[] { 0x7D0E1800, /* 927.25MHz 10 */
	...
	private int[] twFreqSortedIdx = new int[] { 10, 5, 1, 6, 11, 4, 8, 2, 9, 0,
			7, 3, };

	private int[] cnFreqTable = new int[] { 0xD31c3000, /* 922.375MHz */
	...
	private int[] cnFreqSortedIdx = new int[] { 7, 6, 4, 0, 10, 14, 3, 1, 9, 8,
			2, 13, 12, 11, 5, 15, };
